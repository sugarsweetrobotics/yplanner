...
SCENARIO( "Constmap test", "[constmap]" ) {
  GIVEN("Simplest Map") {
    auto map = yplanner::createMap({{0.1, 0.2}, 0.0}, {1, 1}, {0.1, 0.1});
    THEN("Map is valid") {
    ...
    THEN("Map GXWX valid") {
      auto p = yplanner::gridPoseToWorldPose(map->config, {0, 0});
      REQUIRE(p.x == 0.15_a);
      REQUIRE(p.y == 0.15_a);
    }
  ...
  GIVEN("Rotated Map coordinate convert test.") {
    auto map = yplanner::createMap({{0.1, 0.2}, M_PI/2}, {1, 1}, {0.1, 0.1});
    THEN("Map is valid") {
      REQUIRE(map->config.cellsSize.width == 10);
      REQUIRE(map->config.cellsSize.height == 10);
    }
    ...
    THEN("Map GXWX validd with rotation") {
      auto p = yplanner::gridPoseToWorldPose(map->config, {1, 0});
      REQUIRE(p.x == 0.15_a);
      REQUIRE(p.y == 0.35_a);
    }
